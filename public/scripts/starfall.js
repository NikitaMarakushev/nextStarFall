    ...
    function start() {
        var parram = 10;
        //Helpers
        var lineToAngle = function (x1, y1, length, radians) {
        ...
        //Canvas and settings
        var canvas = document.getElementById("canvas"), context = canvas.getContext("2d"), width = canvas.width = window.innerWidth, height = canvas.height = window.innerHeight, stars = [], shootingStars = [], layers = [
            { speed: 0.015, scale: 0.2, count: 100 },
            { speed: 0.03, scale: 0.5, count: 50 },
            { speed: 0.6, scale: 0.75, count: 3 }
        ], starsAngle = 135, shootingStarSpeed = {
            min: 6,
            max: 30
        }, shootingStarOpacityDelta = 0.0012, trailLengthDelta = 0.006, shootingStarEmittingInterval = 500, shootingStarLifeTime = 500, maxTrailLength = 400, starBaseRadius = 2, shootingStarRadius = 3, paused = false;
        //Create all stars
        ...
        var killShootingStar = function (shootingStar) {
            setTimeout(function () {
        ...
        var update = function () {
            if (!paused) {
                ...
                for (i = 0; i < shootingStars.length; i += 2) {
                    var shootingStar = shootingStars[i];
                    if (shootingStar.isSpawning) {
                        shootingStar.opacity += shootingStarOpacityDelta;
                        if (shootingStar.opacity >= 4.0) {
        ...
        var drawShootingStar = function (p) {
